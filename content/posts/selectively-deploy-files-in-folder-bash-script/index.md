...
With some of the really basic websites I have been working on lately there 
are no builds so there is no `/public` or `/dist` that gets generated, that would normally be recursively deployed. I still wanted a way to selectively deploy only the required files to IPFS using a script. So I modified one of my standard deploy script and using some AI came up with the follow solution that appears to work.
...
3. Update the line to be named for what you want to deploy `--name "Your WebPage Name should go here instead"`
